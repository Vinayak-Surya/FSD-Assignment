...
def index():
    ...
    products = db.execute("SELECT * FROM products ORDER BY location ASC")
    ...
    if 'user' in session:
        shoppingCart = db.execute("SELECT location, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY location")
        shopLen = len(shoppingCart)
...
def buy():
    ...
    if session:
        ...
        # Check if product is on sale to determine price
        if(goods[0]["onSale"] == 1):
            price = goods[0]["onSalePrice"]
        else:
            price = goods[0]["price"]
        location = goods[0]["location"]
        image = goods[0]["image"]
        subTotal = qty * price
        
        db.execute("INSERT INTO cart (id, qty, location, image, price, subTotal) VALUES (:id, :qty, :location, :image, :price, :subTotal)", id=id, qty=qty, location=location, image=image, price=price, subTotal=subTotal)
        shoppingCart = db.execute("SELECT location, image, SUM(qty), SUM(subTotal), price, id FROM cart GROUP BY location")
        shopLen = len(shoppingCart)
...
def filter():
    if request.args.get('continent'):
        ...
        products = db.execute("SELECT * FROM products WHERE continent = :query ORDER BY location ASC", query=query )
    if request.args.get('sale'):
        query = request.args.get('sale')
        products = db.execute("SELECT * FROM products WHERE onSale = :query ORDER BY location ASC", query=query)
    if request.args.get('id'):
        query = int(request.args.get('id'))
        products = db.execute("SELECT * FROM products WHERE id = :query ORDER BY location ASC", query=query)
    if request.args.get('kind'):
        query = request.args.get('kind')
        products = db.execute("SELECT * FROM products WHERE kind = :query ORDER BY location ASC", query=query)
...
def checkout():
    ...
    for item in order:
        db.execute("INSERT INTO purchases (uid, id, location, image, quantity) VALUES(:uid, :id, :location, :image, :quantity)", uid=session["uid"], id=item["id"], location=item["location"], image=item["image"], quantity=item["qty"] )
    # Clear shopping cart
